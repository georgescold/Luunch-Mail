...
async function handle(type: JobType, payload: any): Promise<unknown> {
  switch (type) {
    case "send_email":
      return (await import("@/lib/email/messaging")).performScheduledSend(payload.messageId);
    case "simulate_events":
      return (await import("@/lib/email/messaging")).simulateEvents(payload.messageId);
    case "deliver_webhook":
      return (await import("@/lib/email/webhooks")).deliverWebhook(payload.deliveryId);
    case "process_sequences":
      return (await import("@/lib/outreach/outreach")).processSequences();
    case "run_warmup":
      return (await import("@/lib/deliverability/warmup")).runWarmupTick();
    case "verify_dns":
      return (await import("@/lib/deliverability/dns")).verifyDomainRecords(payload.domainId);
    case "run_placement_test":
      return (await import("@/lib/deliverability/deliverability")).completePlacementTest(payload.testId);
    case "poll_imap":
      return (await import("@/lib/email/imap")).pollAllMailboxes();
    case "sync_cheapinboxes":
      return (await import("@/lib/integrations/cheapinboxes")).syncAllCheapInboxes();
    default:
